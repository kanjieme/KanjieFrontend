  ...
  <div class="min-h-screen px-6 py-24">
    <div class="max-w-3xl mx-auto">
      ...
      <div class="flex items-center gap-2 font-mono text-xs text-ash-muted mb-8">
        <NuxtLink :to="`/learn/${levelCode.toLowerCase()}`" class="hover:text-gold transition-colors">
          JLPT {{ levelCode }}
        </NuxtLink>
        <span>→</span>
      ...
      <div v-else-if="lesson">
        ...
        <!-- Markdown content -->
        <div class="prose prose-invert max-w-none mb-12 text-ash-muted leading-relaxed"
...
function renderMarkdown(md: string): string {
  return md
    ...
    .replace(/`(.*?)`/g, '<code class="bg-ink-soft text-gold px-1.5 py-0.5 font-mono text-sm rounded">$1</code>')
    .replace(/^\| (.*)/gm, (_, row) => {
      const cells = row.split(' | ').map((c: string) => `<td class="px-4 py-2 border border-gold/10 text-ash-muted text-sm">${c}</td>`).join('')
      return `<tr>${cells}</tr>`
    })
    .replace(/(<tr>.*<\/tr>\n?)+/gs, (rows) => `<table class="w-full border-collapse my-4">${rows}</table>`)
    .replace(/^---$/gm, '<hr class="border-gold/20 my-6"/>')
    .replace(/\n\n/g, '</p><p class="mb-4">')
    .replace(/^(?!<)(.+)$/gm, '<p class="mb-4 text-ash-muted leading-relaxed">$1</p>')
}
...
onMounted(async () => {
  ...
  if (lessonData) {
    ...
    module.value = lessonData.modules
    renderedContent.value = renderMarkdown(lessonData.content_md || '')
    ...
    const { data: next } = await supabase
      ...
      .gt('order_num', lessonData.order_num)
      .order('order_num')
      .limit(1)
      .single()
